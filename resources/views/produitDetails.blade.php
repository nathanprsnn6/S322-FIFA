    ...
    <form action="{{ route('produits.index') }}" method="POST">
        ...
        <div id="product-info">
            ...
            <div>
                ...
                <label id="lab_size" class="text_detail" for="size_{{ $tailles->first()->idtaille }}"><b>Taille :</b></label>
                    @foreach($tailles as $taille)
                        <input type="radio" id="size_{{ $taille->idtaille }}" name="size" value="{{ $taille->idtaille }}" style="display:none;">
                        <label class="size-button" for="size_{{ $taille->idtaille }}" data-stock="{{ $stocks[$taille->idtaille] ?? 0 }}">
            ...
            <br>
            <div class="add-cart">
                <button type="submit" id="add-button" class="text_detail">Ajouter au panier</button>
            </div>
        </div>
    </form>
</div>
